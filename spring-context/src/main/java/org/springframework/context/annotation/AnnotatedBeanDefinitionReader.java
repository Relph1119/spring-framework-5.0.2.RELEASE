...
public class AnnotatedBeanDefinitionReader {
	...
	<T> void doRegisterBean(Class<T> annotatedClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		...
		// 根据指定的注解Bean定义类，创建Spring容器中对注解Bean的封装的数据类型
		...
		// 解析注解Bean定义的作用域，若@Scope("property")，则Bean为原型类型
		...
		// 主要配置autowiring 自动依赖注入装配的限定条件，即@Qualifier注解
		...
		if (qualifiers != null) {
			for (Class<? extends Annotation> qualifier : qualifiers) {
				...
				// 如果使用了除@Primary 和@Lazy以外的其他注解，则为该Bean添加一个autowiring自动依赖注入装配限定符，
